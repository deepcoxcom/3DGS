...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", is_train=True):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            try:
                # 加载人物掩膜
                people_mask_path = image_path.replace("images", "mask")[:-4]+".npy"
                people_mask = np.load(people_mask_path).astype(np.uint8)
              
                print(f"People mask loaded from {people_mask_path}")
            except FileNotFoundError:
                print(f"People mask not found at {people_mask_path}, generating empty mask.")
        
                people_mask = np.zeros_like(sky_mask, dtype=np.uint8) 
                print("People mask generated as empty (no people detected).")

            sky_mask = sky_mask | people_mask  
