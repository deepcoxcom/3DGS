...
__device__ float ComputeBilateralSSIM(const cudaTextureObject_t ref_image, const Camera ref_camera, const cudaTextureObject_t src_image, const Camera src_camera, const int2 p, const float4 plane_hypothesis, const PatchMatchParams params)
{
    const float cost_max = 2.0f;
    int radius = params.patch_size / 2;
    ...
    float cost = 0.0f;
    {
        ...
        // 计算 SSIM 参数
        const float C1 = 1e-4f; // 稳定项，防止除零
        const float C2 = 9e-4f;

        const float mean_ref = sum_ref;
        const float mean_src = sum_src;
        const float var_ref = sum_ref_ref - mean_ref * mean_ref;
        const float var_src = sum_src_src - mean_src * mean_src;
        const float covar_ref_src = sum_ref_src - mean_ref * mean_src;

        const float ssim_numerator = (2.0f * mean_ref * mean_src + C1) * (2.0f * covar_ref_src + C2);
        const float ssim_denominator = (mean_ref * mean_ref + mean_src * mean_src + C1) * (var_ref + var_src + C2);

        if (var_ref < 1e-5f || var_src < 1e-5f || ssim_denominator <= 0.0f) {
            return cost_max;
        }

        // SSIM 计算值
        const float ssim = ssim_numerator / ssim_denominator;

        // 将 SSIM 转化为 cost，范围 [0, cost_max]
        return cost = max(0.0f, min(cost_max, 1.0f - ssim));
    }
}
...
__device__ float ComputeMultiViewInitialCostandSelectedViews(const cudaTextureObject_t *images, const Camera *cameras, const int2 p, const float4 plane_hypothesis, unsigned int *selected_views, const PatchMatchParams params)
{
    ...
    for (int i = 1; i < params.num_images; ++i) {
        float c = ComputeBilateralSSIM(images[0], cameras[0], images[i], cameras[i], p, plane_hypothesis, params);
        cost_vector[i - 1] = c;
...
{
    for (int i = 1; i < params.num_images; ++i) {
        cost_vector[i - 1] = ComputeBilateralSSIM(images[0], cameras[0], images[i], cameras[i], p, plane_hypothesis, params);
    }
}
