...
from gaussian_renderer import render, network_gui, render_with_mask
...
def save_image(image, iteration, output_path):
    ...
    imageio.imwrite(os.path.join(output_path, f"image_{iteration}.png"), image)  
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        person_mask = viewpoint_cam.sky_mask.cuda()  

        
        person_masks = ~person_mask.to(torch.bool)  

        render_pkg = render_with_mask(viewpoint_cam, gaussians, pipe, bg, person_masks, 
        ...
        if opt.normal_loss:
            ...
            if viewpoint_cam.normal is not None:
                ...
                normal_gt = F.normalize(normal_gt, dim=0)
                
                
                l1_normal = torch.abs(rendered_normal - normal_gt).sum(dim=0)[filter_mask].mean()
                cos_similarity = (1. - torch.sum(rendered_normal * normal_gt, dim=0))[filter_mask].mean()

                
                # Additional consistency term (e.g., angle difference constraint)
                angle_diff = torch.acos(torch.clamp(torch.sum(rendered_normal * normal_gt, dim=0), -1.0, 1.0))
                
                angular_loss = angle_diff[filter_mask].mean()
    
                # angular_loss = (1 - torch.sum(rendered_normal * normal_gt, dim=0) ** 2)[filter_mask].mean()

                
                # Combine losses
                loss += opt.lambda_normal*(
                    opt.lambda_l1_normal * l1_normal +
                    opt.lambda_cos_normal * cos_similarity +
                    opt.lambda_angular_normal * angular_loss  # New term
                )
        ...
        with torch.no_grad():
            ...
            # Save image every 1000 iterations
            if iteration % 1000 == 0: 
        
   
                if opacity_mask.ndimension() == 4:
                    opacity_mask = opacity_mask.squeeze(0)  
                
               
               
                save_image(image * opacity_mask, iteration, './saved_images')  
                save_image(gt_image, iteration, './saved_gt_images')
                print("opacity_mask shape:", opacity_mask.shape)
                print("image shape:", image.shape)
                print("gt_image shape:", gt_image.shape)
                print("opacity_mask min:", opacity_mask.min(), " max:", opacity_mask.max())
